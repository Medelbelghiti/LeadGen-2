...
export const POST = withErrorHandling(async (req) => {
  ...
  const activeCount = await db.vehicle.count({ where: { userId: user.id, archived: false } });
  if (activeCount >= ent.maxVehicles) {
    throw new LimitReachedError("vehicles", `Plan limit reached (${ent.maxVehicles} vehicles). Upgrade to add more.`);
  }

  if (body.isPrimary) {
    await db.vehicle.updateMany({ where: { userId: user.id, isPrimary: true }, data: { isPrimary: false } });
  }
