...
export function BillingClient(props: {
  ...
  const cancel = async (atPeriodEnd: boolean) => {
    ...
    setB(false);
    setMsg(r.ok ? "Cancellation processed" : (await r.json()).error);
    router.refresh();
  };
  ...
  if (props.isPlanPicker) {
    return (
      <div className="mt-2">
        <input className="input mb-2" placeholder="Coupon code (optional)" value={coupon} onChange={(e) => setC(e.target.value)} />
        <button onClick={checkout} disabled={props.disabled || busy} className="btn btn-secondary text-sm w-full">
          {props.disabled ? "Current plan" : busy ? "…" : `Switch to ${props.planName}`}
        </button>
